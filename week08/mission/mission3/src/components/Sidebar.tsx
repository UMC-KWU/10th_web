...
export const Sidebar = ({ isOpen, onClose }: SidebarProps) => {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("keydown", handleEscape); //cleanup 함수
    };
  }, [isOpen, onClose]);

  //2. 사이드바 열려있으면 본문 내용 스크롤 안 되게 막기
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <div
      className={`fixed inset-0 bg-black/50 backdrop-blur-sm 
        trasition-opacity duration-300 z-70 ${isOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
      onClick={onClose}
    >
      <aside
